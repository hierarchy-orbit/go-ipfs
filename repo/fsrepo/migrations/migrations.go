...
func RunMigration(ctx context.Context, targetVer int, ipfsDir string) error {
	...
	if len(binPaths) < len(migrations) {
		...
		tmpDir, err := ioutil.TempDir("", distMigsRoot)
		if err != nil {
			return err
		}
...
func findMigrations(ctx context.Context, from, to int) ([]string, map[string]string, error) {
	...
	for cur := from; cur != to; cur += step {
		...
		bin, err := exec.LookPath(exeName(migName))
		if err != nil {
			continue
		}
...
func fetchMigrations(ctx context.Context, needed []string, destDir string) ([]string, error) {
	...
	ver, err := LatestDistVersion(ctx, distFSRM)
	if err != nil {
		return nil, err
	}
	...
	for i, name := range needed {
		...
		go func(i int, name string) {
			loc, err := FetchBinary(ctx, distMigsRoot, ver, name, name, destDir)
			if err != nil {
	...
	if len(fails) != 0 {
		return nil, fmt.Errorf("failed to download migrations: %s", strings.Join(fails, " "))
	}
	return bins, nil
}
